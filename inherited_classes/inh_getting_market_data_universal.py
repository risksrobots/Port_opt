...
class MoscowExchangeData(AbstractGetPreprocMarketData):
    ...
    def get_data(self, market):
        ...
        if market not in self.dict_urls.keys():
            raise ValueError("there are not this market in urls")

        dict_market_prices = dict()
        for ticker in self.tickers:

            url0 = self.dict_urls[market] + str(ticker) \
                    + '.csv' + '?from=' + self.start_date + '&till=' \
                    + self.finish_date + '&start={}'

            slice_end = self.dict_slices[market]
